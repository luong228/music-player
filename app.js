...
const app = {
    ...
    handleEvents: function (isMusic) {
        ...
        animateThumb.pause();
        this.handleAlbumMusic(animateThumb)
        ...
        playlist.onclick = function (e) {
            ...
            if(e.target.closest('.option')) {
                ...
                _this.handleLikeSongs(opt, isMusic, animateThumb)
            }
    ...
    // Handle Like Song
    handleLikeSongs: function (dataSongs, isMusic, animateThumb) {
        ...
        $('.like').onclick = function (e) {
            if(e.target.getAttribute('value') == 'like') {
                ...
                newArray = _this.songs.love.filter(function(value) {
                    return value.name != name;
                })
    ...
    // Handle Album Music Events
    handleAlbumMusic: function(animateThumb) {
        const _this = this;
        ...
        $$('.locationmusic div').forEach(function(value) {
            value.onclick = function(e) {
                ...
                _this.checkCurrentIndex(isMusic);
                
                if(!e.target.closest('.locationmusic > .select')) {
                    if(valueIsMusic == 'love' && _this.songs.love.length < 1) {
                        ...
                        _this.config[isMusic] == undefined ? _this.setConfig(isMusic, 0) : ''
                        _this.config[isMusic] ? _this.currentIndex = _this.config[isMusic] : _this.currentIndex = 0;
                        indexArray = []
                        progress.value = 0
                        animateThumb.pause();
                        _this.isPlaying = false;
    ...
    randomSong: function (isMusic) {
        ...
        if(indexArray.length === this.songs[isMusic].length) {
            indexArray = []
        }
        do {
            newIndex = Math.floor(Math.random() * this.songs[isMusic].length);
        }
        while(indexArray.includes(newIndex))
        
        indexArray.push(newIndex);
        this.config[isMusic] = newIndex;
        this.loadCurrentSong(isMusic);
